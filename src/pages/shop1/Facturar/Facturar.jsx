...
const Facturar = () => {
    ...
    if(oneProduct.stock[0].stock >= products.cantidad){
      if (products.cantidad !== "" && products.productId !== "") {
        if( list.length === 0 ) {
            ...
            const validate = list.map((item) => item.productId === products.productId);
  ...
  const addFact = () => {
    const values = {
      montoPagado: totalRecibido,
      tipoFacturaId: Number(tipoFac),
      usuarioId: Access.id,
      tipoAlmacenId: 2,
    }

    const { montoPagado, tipoFacturaId, usuarioId, tipoAlmacenId } = values;

    dispatch(createBill(total, montoPagado, tipoFacturaId, usuarioId, tipoAlmacenId, data));
  }
  ...
  return (
    <>
      ...
      <div className="mx-auto w-11/12 lg:pl-12 py-24">
        ...
        <div>
            <h4 className='text-2xl font-bold mb-6'>Seleccione cliente</h4>
            <div className='flex flex-row items-end gap-6'>
              ...
              <div>
                <button className='bg-orange hover:bg-hover-orange text-white font-bold py-2 px-4 rounded-lg'>Añadir</button>
              </div>
            </div>
          </div>       
          <div className='mt-8'>
            ...
            <div className='flex justify-end mt-4'>

              {
                buttonState ? (
                  ...
                  <div
                    className='bg-gray-200 text-white font-bold py-2 px-4 rounded-lg'
                  >
                    Agregar
                  </div>
                )
          ...
          <div className="mt-6">
              {list.length > 0 && (
                <div className='border-2 rounded-lg p-8'>
                  <h3 className='text-3xl mb-8'>Factura</h3>
                  ...
                  <div className='flex flex-row my-6 px-4 justify-between'>
                    <div>
                      ...
                      <div>
                        <label className='text-xl font-semibold'>Tipo de factura</label>
                        <select
                    ...
                    <div className='w-80 border p-4 rounded-3xl'>
                      <h3 className='text-xl font-semibold py-2 text-center'>Detalles del pago</h3>
                      ...
                      <div className='py-2 flex flex-row justify-between'>
                        ...
                        <span className='font-light'>C${totalRecibido - total}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-end justify-end mt-10 gap-8">
                    {
                      facButton ? (
                        ...
                        <div
                          className="bg-gray-200 text-white font-bold py-[10px] px-4 rounded-lg"
                        >
                          Facturar
                        </div>
                      )
